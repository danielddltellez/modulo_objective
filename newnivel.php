...
$nivelform = new newnivels_form();
$toform['userid'] = $USER->id;
$toform['idinstance'] = $idinstance;
$toform['courseid'] = $courseid;
$toform['idmod'] = $id;
$nivelform->set_data($toform);


$courseurl = new moodle_url('/mod/objective/viewnivel.php', array('id' => $id));
//Form processing and displaying is done here
if ($nivelform->is_cancelled()) {

  redirect($courseurl);


    //Handle form cancel operation, if cancel button is present on form
} else if ($fromform = $nivelform->get_data()) {
    global $DB;
  //In this case you process validated data. $mform->get_data() returns data posted in form.

  $lastinsertid = $DB->insert_record('objective_nivel', $fromform);
